...
def validar_tempo(tempo):
    ...
    if not (0 <= minutos <= limite_minutos_segundos):
        print(
            f'Erro: Minutos devem estar entre 0 e {limite_minutos_segundos}.'
        )
        return False

    if not (0 <= segundos <= limite_minutos_segundos):
        print(
            f'Erro: Segundos devem estar entre 0 e {limite_minutos_segundos}.'
        )
        return False
...
obter_tempo_em_segundos = lambda tempo: (
    int(tempo.split(':')[0]) * 3600
