...
from carrinho import criar_lista_de_compra, verificacao_do_codigo_de_barra
...
while True:
    ...
    quantidades = {}
    print(
        "Digite os códigos de barras dos produtos (digite '0' para finalizar):"
    )

    while True:
        codigo_de_barras = input('Código de barras: ').strip()
        flag = verificacao_do_codigo_de_barra(
            codigo_de_barras=codigo_de_barras, carrinho=carrinho
        )
        ...
        criar_lista_de_compra(
            lista_de_mercadorias=lista_de_mercadorias,
    ...
    print(
        f"Total da compra com tributos (15.7%):{'_' * 59}R$ {compra["total_com_tributo"]:.2f}"
    )
    if cpf:
    ...
    total_com_tributo = (
        compra['total_com_desconto'] * configuracoes['taxa_tributo']
    )
    ...
    print(
        f"Total da compra com os descontos: R$ {compra["total_com_desconto"] / 100:.2f}"
    )
    print(
